...
void cBulletFurnace::_createStaticObjects() {
    // define objects
...
void cBulletFurnace::_createEndEffector() {
...
void cBulletFurnace::setEndEffectorPose(cVector3d position, cMatrix3d rotation) {
...
bool cBulletFurnace::updateShadowMaps(const bool mirror_x, const bool mirror_y) {
    return _furnaceWorld->updateShadowMaps(mirror_x, mirror_y);
}

void cBulletFurnace::renderView(const int a_windowWidth, const int a_windowHeight, const cEyeMode a_eyeMode, const bool a_defaultBuffer) {
    _camera->renderView(a_windowWidth, a_windowHeight, a_eyeMode, a_defaultBuffer);
}

void cBulletFurnace::computeGlobalPositions(const bool frameOnly, const cVector3d& globalPos, const cMatrix3d& globalRot) {
    _furnaceWorld->computeGlobalPositions(frameOnly, globalPos, globalRot);
}

void cBulletFurnace::updateDynamics(double interval) {
    _furnaceWorld->updateDynamics(interval);
}

void cBulletFurnace::addVisualComponent(cGenericObject* object) {
...
void cBulletFurnace::getCameraPose(cVector3d& eye, cVector3d& target) {
    eye.copyfrom(_cameraEye);
    target.copyfrom(_cameraTarget);
}

void cBulletFurnace::updateCameraPose(cVector3d eye, cVector3d target) {
    _cameraEye.copyfrom(eye);
    _cameraTarget.copyfrom(target);
    _camera->set(eye, target, cVector3d(0.0, 0.0, 1.0));     
}

void cBulletFurnace::moveCamera(cVector3d step) {
...
void cBulletFurnace::rotateCamera(double yawStepInDeg, double pitchStepInDeg) {
    cMatrix3d viewMatrix = rotationMatrixFromVectors(_cameraTarget - _cameraEye, cVector3d(0.0, 0.0, 1.0));
...
void cBulletFurnace::setEndEffectorStiffness(double posStiffness_, double rotStiffness_){
    _linStiffness = posStiffness_;
    _angStiffness = rotStiffness_;
}

void cBulletFurnace::setEndEffectorDamping(double posDamping_, double rotDamping_) {
    _linDamping = posDamping_;
    _angDamping = rotDamping_;
}


void cBulletFurnace::getForceTorqueSensorValue(cVector3d& sensorForce, cVector3d& sensorTorque) {
    for (int i = 0; i < 3; i++) {
