...
void setTextureMapFromFile(cMesh* mesh, string fileName, double scale, double maxSize = 20.0) {
	...
	for (unsigned int i = 0; i < numTriangles; i++) {
		if (mesh->m_triangles->m_allocated[i]) {
			...
			cVector3d pos0 = mesh->m_vertices->getLocalPos(index0) * scale;
			cVector3d pos1 = mesh->m_vertices->getLocalPos(index1) * scale;
			cVector3d pos2 = mesh->m_vertices->getLocalPos(index2) * scale;
