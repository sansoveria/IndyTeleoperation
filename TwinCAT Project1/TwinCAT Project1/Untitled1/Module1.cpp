...
HRESULT CModule1::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (SUCCEEDED(ipTask->GetCycleCounter(&cnt)))
	{
		...
		theta1 = float(m_Inputs.Position1 - m_DeviceStates.AngleZero1) * 2.0 * TC_M_PI / 4096.0 / 5.0;
		theta2 = float(m_Inputs.Position2 - m_DeviceStates.AngleZero2) * 2.0 * TC_M_PI / 4096.0 / 5.0;
		m_DeviceStates.Angle1 = LONGLONG(theta1 / TC_M_PI * 180.0 * 1000.0);
		m_DeviceStates.Angle2 = LONGLONG(theta2 / TC_M_PI * 180.0 * 1000.0);
		c1 = cos_(theta1);
		s1 = sin_(theta1);
		c2 = cos_(theta2);
		s2 = sin_(theta2);
